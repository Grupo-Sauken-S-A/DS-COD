...
public class DsDJO
{
    ...
    private static String URIDJO = null;
    private static String URIDJOEH = null;
    private final int djoID = 0;
    private final int djoehID = 1;
     ...
     */
    public DsDJO(String data, int token, String outputMsg) throws CustomException, IOException {
        if (token == 1) {
    ...
    private static void showHelp() {
        ...
        System.out.println("                            [--soft]    [path-al-cert]         [DJO/DJOEH] [documento] [output] [output-msg] [output-error]");
        System.out.println("                            [--token]   [config-con-dll] [pin] [DJO/DJOEH] [documento] [output] [output-msg] [output-error]");
        System.out.println();
        ...
        System.out.println();
        System.out.println("        DJO/DJOEH                            Elegir como firmar");
    ...
    public static void main(String[] args)
        ...
        String startup = "DS-DJO versión 1.2.3\n" +
                "Derechos Reservados © 2024 Grupo Sauken S.A.\n" +
                "Copyright © 2024 Grupo Sauken S.A.\n\n" +
                "DS-DJO carece totalmente de garantía. Este es Software Libre y\n" +
        ...
        String licencia = " Derechos Reservados © 2024 Grupo Sauken S.A.\n" +
                ...
                " 51 Franklin Street, Fifth Floor, Boston, MA 02110-1301 USA.\n" +
                "\n" +
                " Author: Grupo Sauken S.A\n" +
                " E-mail: mailto:soporte@sauken.com\n" +
                " Company: Grupo Sauken S.A.\n" +
                " WebSite: https://www.sauken.com.ar/" +
                "\n";
        ...
        if (isToken == 1)
        {
            ...
            djo = args[3].equals("DJO");
            djoeh = args[3].equals("DJOEH");
            if (!((!djo && djoeh) || (djo && !djoeh))) {
                if (outputMsg != null) {
                    List<String> lines = Arrays.asList("No es posible firmar el nodo: " + args[3], "Por favor, use DJO o DJOEH solamente");
                    Path file = Paths.get(outputMsg);
                    Files.write(file, lines, Charset.forName("UTF-8"), StandardOpenOption.APPEND);
                    return;
                } else {
                    System.out.println("No es posible firmar el nodo: " + args[3]);
                    System.out.println("Por favor, use DJO o DJOEH solamente");
                    return;
                }
            }
            ...
            DsDJO genEnvel = new DsDJO(dll, isToken, outputMsg);
            genEnvel.selectSignatureKey(args[2]);
            ERROR = genEnvel.createXmlSignature(args[3], doc, djo, djoeh, docName, outputMsg);
            if (!ERROR) {
                genEnvel.writeResult(args, isToken, djo, djoeh, doc, docName, outputMsg);
            } else {
                if (outputMsg != null) {
            ...
            djo = args[2].equals("DJO");
            djoeh = args[2].equals("DJOEH");
            if (!((!djo && djoeh) || (djo && !djoeh))) {
                if (outputMsg != null) {
                    List<String> lines = Arrays.asList("No es posible firmar el nodo: " + args[2], "Por favor, use DJO o DJOEH solamente");
                    Path file = Paths.get(outputMsg);
                    Files.write(file, lines, Charset.forName("UTF-8"), StandardOpenOption.APPEND);
                    return;
                } else {
            ...
            DsDJO genEnvel = new DsDJO(fileName, isToken, outputMsg);
            ...
            ERROR = genEnvel.createXmlSignature(args[2], doc, djo, djoeh, docName, outputMsg);
            if (!ERROR) {
                genEnvel.writeResult(args, isToken, djo, djoeh, doc, docName, outputMsg);
            } else {
                if (outputMsg != null) {
    ...
    private static boolean verifySignature(Document doc, String docName, int id, String outputMsg) throws Exception {
        ...
        if (nl.getLength() == 0) {
            if (outputMsg != null) {
                List<String> lines = Collections.singletonList(docName + ": No se puede encontrar la firma" + "URIDJO: " + URIDJO + ", URIDJOEH: " + URIDJOEH);
                Path file = Paths.get(outputMsg);
                Files.write(file, lines, Charset.forName("UTF-8"), StandardOpenOption.APPEND);
            } else {
        ...
        if (id == 1) {
            if (nl.getLength() < 2) {
                if (outputMsg != null) {
                    List<String> lines = Collections.singletonList(docName + ": DJOEH no firmado");
                    Path file = Paths.get(outputMsg);
                    Files.write(file, lines, Charset.forName("UTF-8"), StandardOpenOption.APPEND);
                } else {
                    System.out.println(docName + ": DJOEH no firmado");
                }
                return false;
            }
        }
        ...
        if (URI != null && URI.equals("#DJO")) {
            XPathExpression exprDJO = xpath.compile(String.format("//*[@id='%s']", "DJO"));
            NodeList nodesDJO = (NodeList) exprDJO.evaluate(doc, XPathConstants.NODESET);
            Node nodeDJO = nodesDJO.item(0);
            ((Element) nodeDJO).setIdAttribute("id", true);
        } else if (URI != null && URI.equals("#DJOEH")) {
            XPathExpression exprDJOEH = xpath.compile(String.format("//*[@id='%s']", "DJOEH"));
            NodeList nodesDJOEH = (NodeList) exprDJOEH.evaluate(doc, XPathConstants.NODESET);
            Node nodeDJOEH = nodesDJOEH.item(0);
            ((Element) nodeDJOEH).setIdAttribute("id", true);
        }
        ...
        if (!coreValidity) {
            if (id == 0) {
                if (outputMsg != null) {
                    List<String> lines = Collections.singletonList(docName + ": La firma en DJO ha fallado la verificación base");
                    Path file = Paths.get(outputMsg);
                    Files.write(file, lines, Charset.forName("UTF-8"), StandardOpenOption.APPEND);
                } else {
                    System.out.println(docName + ": La firma en DJO ha fallado la verificación base");
                }
                boolean sv = signature.getSignatureValue().validate(valContext);
                if (outputMsg != null) {
                    List<String> lines = Collections.singletonList(docName + ": Estado de validez de la DJO: " + sv);
                    Path file = Paths.get(outputMsg);
                    Files.write(file, lines, Charset.forName("UTF-8"), StandardOpenOption.APPEND);
                } else {
                ...
                for (Object o : signature.getSignedInfo().getReferences()) {
                    ...
                    if (outputMsg != null) {
                        List<String> lines;
                        lines = Collections.singletonList(docName + ": Estado de validez de la DJO: " + refValid);
                        Path file = Paths.get(outputMsg);
                        Files.write(file, lines, Charset.forName("UTF-8"), StandardOpenOption.APPEND);
                    } else {
                        System.out.println(docName + ": Estado de validez de la DJO: " + refValid);
                    }
                }
            } else if (id == 1) {
                if (outputMsg != null) {
                    List<String> lines = Collections.singletonList(docName + ": La firma en DJOEH ha fallado la verificación base");
                    Path file = Paths.get(outputMsg);
                    Files.write(file, lines, Charset.forName("UTF-8"), StandardOpenOption.APPEND);
                } else {
                    System.out.println(docName + ": La firma en DJOEH ha fallado la verificación base");
                }
                boolean sv = signature.getSignatureValue().validate(valContext);
                if (outputMsg != null) {
                    List<String> lines = Collections.singletonList(docName + ": Estado de validez del DJOEH: " + sv);
                    Path file = Paths.get(outputMsg);
                    Files.write(file, lines, Charset.forName("UTF-8"), StandardOpenOption.APPEND);
                } else {
                    System.out.println(docName + ": Estado de validez : " + sv);
                ...
                for (Object o : signature.getSignedInfo().getReferences()) {
                    boolean refValid = ((Reference) o).validate(valContext);
                    if (outputMsg != null) {
                        List<String> lines = Collections.singletonList(docName + ": Estado de validez del DJOEH: " + refValid);
                        Path file = Paths.get(outputMsg);
                        Files.write(file, lines, Charset.forName("UTF-8"), StandardOpenOption.APPEND);
                    } else {
                        System.out.println(docName + ": Estado de validez del DJOEH: " + refValid);
                    }
                }
            }
            return false;
            ...
            if (id == 0) {
                if (outputMsg != null) {
                    List<String> lines = Collections.singletonList(docName + ": La firma en DJO ha pasado la verificación base");
                    Path file = Paths.get(outputMsg);
                    Files.write(file, lines, Charset.forName("UTF-8"), StandardOpenOption.APPEND);
                } else {
                    System.out.println(docName + ": La firma en DJO ha pasado la verificación base");
                }
            } else if (id == 1) {
                if (outputMsg != null) {
                    List<String> lines = Collections.singletonList(docName + ": La firma en DJOEH ha pasado la verificación base");
                    Path file = Paths.get(outputMsg);
                    Files.write(file, lines, Charset.forName("UTF-8"), StandardOpenOption.APPEND);
                } else {
                    System.out.println(docName + ": La firma en DJOEH ha pasado la verificación base");
                }
            }
            return true;
        }
     ...
     * @param djo If is djo
     * @param djoeh If is djoeh
     * @param docName Name of the document
     * @param outputMsg output-msg
     * @return if signed correctly
     * @throws Exception Exception
     */
    private boolean createXmlSignature(String dataURL, Document doc, boolean djo, boolean djoeh, String docName, String outputMsg)
            throws Exception
    {
        getURI(doc);

        if (djo) {
            if (URIDJO != null && URIDJO.equals("#DJOEH")) {
                if (outputMsg != null)
                {
                    List<String> lines = Collections.singletonList(docName + ": Tag DJOEH ya firmado, no es posible firmar tag DJO, verifique el xml");
                    Path file = Paths.get(outputMsg);
                    Files.write(file, lines, Charset.forName("UTF-8"), StandardOpenOption.APPEND);
                } else {
                    System.out.println(docName + ": Tag DJOEH ya firmado, no es posible firmar tag DJO, verifique el xml");
                }
                return true;
            } else if (URIDJO != null && URIDJO.equals("#DJO")) {
                boolean verifiedDJO = verifySignature(doc, docName, djoID, outputMsg);
                if (verifiedDJO) {
                    if (outputMsg != null)
                    {
                        List<String> lines = Collections.singletonList(docName + ": Tag DJO ya firmado de manera válida");
                        Path file = Paths.get(outputMsg);
                        Files.write(file, lines, Charset.forName("UTF-8"), StandardOpenOption.APPEND);
                    } else {
                        System.out.println(docName + ": Tag DJO ya firmado de manera válida");
                    }
                    return true;
                } else {
                    if (outputMsg != null)
                    {
                        List<String> lines = Collections.singletonList(docName + ": Tag DJO ya firmado de manera inválida, verifique el xml");
                        Path file = Paths.get(outputMsg);
                        Files.write(file, lines, Charset.forName("UTF-8"), StandardOpenOption.APPEND);
                    } else {
                        System.out.println(docName + ": Tag DJO ya firmado de manera inválida, verifique el xml");
                    }
                    return true;
                }
            }
            if (URIDJOEH != null && URIDJOEH.equals("#DJOEH")) {
                boolean verifiedDJOEH = verifySignature(doc, docName, djoehID, outputMsg);
                if (verifiedDJOEH) {
                    if (outputMsg != null)
                    {
                        List<String> lines = Collections.singletonList(docName + ": Tag DJOEH ya firmado de manera válida, no es posible firmar tag DJO");
                        Path file = Paths.get(outputMsg);
                        Files.write(file, lines, Charset.forName("UTF-8"), StandardOpenOption.APPEND);
                    } else {
                        System.out.println(docName + ": Tag DJOEH ya firmado de manera válida, no es posible firmar tag DJO");
                    }
                    return true;
                }
            }
        } else if (djoeh) {
            if (URIDJO != null && URIDJO.equals("#DJO")) {
                boolean verifiedDJO = verifySignature(doc, docName, djoID, outputMsg);
                if (verifiedDJO) {
                    if (outputMsg != null)
                    {
                        List<String> lines = Collections.singletonList(docName + ": Tag DJO firmado de manera válida");
                        Path file = Paths.get(outputMsg);
                        Files.write(file, lines, Charset.forName("UTF-8"), StandardOpenOption.APPEND);
                    } else {
                        System.out.println(docName + ": Tag DJO firmado de manera validá");
                    }
                } else {
                    if (outputMsg != null)
                    {
                        List<String> lines = Collections.singletonList(docName + ": Tag DJO firmado de manera inválida, verifique el xml");
                        Path file = Paths.get(outputMsg);
                        Files.write(file, lines, Charset.forName("UTF-8"), StandardOpenOption.APPEND);
                    } else {
                        System.out.println(docName + ": Tag DJO firmado de manera inválida, verifique el xml");
                    }
                    return true;
                }
            } else {
                if (outputMsg != null)
                {
                    List<String> lines = Collections.singletonList(docName + ": Tag DJO no firmado, verifique el xml");
                    Path file = Paths.get(outputMsg);
                    Files.write(file, lines, Charset.forName("UTF-8"), StandardOpenOption.APPEND);
                } else {
                    System.out.println(docName + ": Tag DJO no firmado, verifique el xml");
                }
                return true;
            }
            if (URIDJOEH != null && URIDJOEH.equals("#DJOEH")) {
                ...
                if (verifiedDJOEH) {
                    if (outputMsg != null)
                    {
                        List<String> lines = Collections.singletonList(docName + ": Tag DJOEH ya firmado de manera válida, no es posible volver a realizar la firma");
                        Path file = Paths.get(outputMsg);
                        Files.write(file, lines, Charset.forName("UTF-8"), StandardOpenOption.APPEND);
                    } else {
                        System.out.println(docName + ": Tag DJOEH ya firmado de manera válida, no es posible volver a realizar la firma");
                    }
                    return true;
                } else {
                    if (outputMsg != null)
                    {
                        List<String> lines = Collections.singletonList(docName + ": Tag DJOEH ya firmado de manera inválida, verifique el xml");
                        Path file = Paths.get(outputMsg);
                        Files.write(file, lines, Charset.forName("UTF-8"), StandardOpenOption.APPEND);
                    } else {
                        System.out.println(docName + ": Tag DJOEH ya firmado de manera inválida, verifique el xml");
                    }
                    return true;
                }
            }
    ...
    private void getURI(Document doc) {
        try {
            ...
            NodeList nl = doc.getElementsByTagNameNS(XMLSignature.XMLNS,
                    "Signature");
            if (nl.getLength() != 0) {
                // Get the djo reference URI
                Element nodeElementDJO = (Element) nl.item(djoID);
                NodeList sigInfoDJO = nodeElementDJO.getElementsByTagNameNS(XMLSignature.XMLNS, "SignedInfo");
                Element sigElementDJO = (Element) sigInfoDJO.item(0);
                NodeList referenceURIDJO = sigElementDJO.getElementsByTagNameNS(XMLSignature.XMLNS, "Reference");
                Element referenceElementDJO = (Element) referenceURIDJO.item(0);
                URIDJO = referenceElementDJO.getAttributes().getNamedItem("URI").getNodeValue();

                if (URIDJO.equals("#DJOEH")) {
                    URIDJOEH = URIDJO;
                    return;
                }
                // Get the DJOEH reference URI
                Element nodeElementDJOEH = (Element) nl.item(djoehID);
                NodeList sigInfoDJOEH = nodeElementDJOEH.getElementsByTagNameNS(XMLSignature.XMLNS, "SignedInfo");
                Element sigElementDJOEH = (Element) sigInfoDJOEH.item(0);
                NodeList referenceURIDJOEH = sigElementDJOEH.getElementsByTagNameNS(XMLSignature.XMLNS, "Reference");
                Element referenceElementDJOEH = (Element) referenceURIDJOEH.item(0);
                URIDJOEH = referenceElementDJOEH.getAttributes().getNamedItem("URI").getNodeValue();
            }
        } catch (Exception ignored) {}
    }
     ...
     */
    private void writeResult(String[] args, int isToken, boolean djo, boolean djoeh, Document doc, String docName, String outputMsg)
            throws Exception {
        ...
        if (djo) {
            if (outputMsg != null)
            {
                List<String> lines = Collections.singletonList(docName + ": DJO firmado correctamente");
                Path file = Paths.get(outputMsg);
                Files.write(file, lines, Charset.forName("UTF-8"), StandardOpenOption.APPEND);
            } else {
                System.out.println(docName + ": DJO firmado correctamente");
            }
        } else if (djoeh) {
            if (outputMsg != null)
            {
                List<String> lines = Collections.singletonList(docName + ": DJOEH firmado correctamente");
                Path file = Paths.get(outputMsg);
                Files.write(file, lines, Charset.forName("UTF-8"), StandardOpenOption.APPEND);
            } else {
                System.out.println(docName + ": DJOEH firmado correctamente");
            }
        }
    }
